<template>
  <div class="con-license">
    ...
    <div id="plans" class="con-plans">
      ...
      <div class="con-cards">
        <div
          v-for="(card, i) in pageFrontmatter.cards?.cards"
          :key="i"
          class="card"
        >
    ...
    <div class="table-plans">
      <h2>{{ pageFrontmatter.table?.title }}</h2>

      <div class="con-tabla">
        <table>
          <thead>
            <tr>
              <th>{{ pageFrontmatter.table?.thead[0] }}</th>
              <th>{{ pageFrontmatter.table?.thead[1] }}</th>
              <th>{{ pageFrontmatter.table?.thead[2] }}</th>
              <th>{{ pageFrontmatter.table?.thead[3] }}</th>
            </tr>
          </thead>
          <tbody>
            <tr
              v-for="(tr, index) in pageFrontmatter.table?.tbody"
              :key="index"
            >
              <td>{{ tr[0] }}</td>
    ...
    <div id="faq" class="con-faq">
      <h2>Got Questions? Check the FAQs</h2>
      <ul>
...
.darken {
  .con-plans {
    .card {
      &:hover {
        box-shadow: 0px 15px 40px 0px rgba(0, 0, 0, 0.7) !important;
      }
    }
  }
}
