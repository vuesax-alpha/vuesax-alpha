<template>
  <main ref="pageRef" class="page">
    <header ref="headerRef" class="header-page">
      <router-link
      ...
      <div class="header__content">
        <div ref="flexRef" class="flex-header">
          <h1 id="header-title" ref="titleRef">
            {{ pageData.title }}
          </h1>

          <ul ref="titleReful" class="interactive-links">
            <li title="Share">
              <button>
    ...
    <div v-if="commitLog?.length" class="content__default">
      <h2>Contributors</h2>

      <ul class="contributors">
        <li v-for="(commit, i) in commitLog" :key="i" class="contributor">
          <vs-avatar size="36" class="contributor__avatar">
            <img :src="commit.committer.avatar_url" alt="" />
          </vs-avatar>
          <h3 class="contributor__name">{{ commit.commit.author.name }}</h3>
        </li>
      </ul>
    </div>
    ...
    <div ref="toTopBtn" class="up" @click="handleUp">
...
const flexRef = ref<HTMLElement>()
const titleRef = ref<HTMLElement>()
const titleReful = ref<HTMLElement>()
const toTopBtn = ref<HTMLElement>()

const filepath = computed(() => {
  const path = pageData.value.path
  if (extRE.test(path)) return path
  return `${path}README.md`
})

const { data: commitLog } = useCommitLog(filepath)
...
useEventListener('scroll', () => {
  if (toTopBtn.value) {
    if (window.pageYOffset > 300) {
      toTopBtn.value.classList.add('active')
    } else {
      toTopBtn.value.classList.remove('active')
    }
  }

  if (window.pageYOffset > 200) {
    pageRef.value?.querySelector('.sidebar')?.classList.add('fixed')
  } else {
    pageRef.value?.querySelector('.sidebar')?.classList.remove('fixed')
  }

  if (titleRef.value && flexRef.value) {
    // what is 20, 8, 35, 24, 7, 2, 70 ??????
    titleRef.value.style.fontSize = `${
      35 - window.pageYOffset / 7 > 24 ? 35 - window.pageYOffset / 8 : 20
    }px`

    flexRef.value.style.marginBottom = `${
  ...
  if (headerRef.value) {
    ...
    if (window.pageYOffset > 140) {
      ...
      headerRef.value.style.width = `${pageRef.value?.offsetWidth}px`
    } else {
